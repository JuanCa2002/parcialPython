...
from juegoMillonario.Infraestructura.persistenciaCategoria import PersistenciaCategoria
from juegoMillonario.Infraestructura.persistenciaHistorial import PersistenciaHistorial

saver = Persistencia()
saver.connect()
saverCategoria = PersistenciaCategoria()
saverCategoria.connect()
saverHistorial=PersistenciaHistorial()
saverHistorial.connect()



def generarCategorias():
    categorias = saverCategoria.consultar_tabla_categoria()
    if (len(categorias) == 0):
        categoriaDeportes = Categoria("1", "Deportes")
        saverCategoria.guardar_categoria(categoriaDeportes)
        categoriaHistoria = Categoria("2", "Historia")
        saverCategoria.guardar_categoria(categoriaHistoria)
        categoriaArte = Categoria("3", "Arte")
        saverCategoria.guardar_categoria(categoriaArte)
        categoriaCiencias = Categoria("4", "Ciencias")
        saverCategoria.guardar_categoria(categoriaCiencias)
        categoriaFiccion = Categoria("5", "Fantasia y ciencia ficcion")
        saverCategoria.guardar_categoria(categoriaFiccion)
            ...
            if respuesta == "1":
                ...
                if opciones[respuesta] == pregunta.opcionCorrecta:
                    ...
                    if contador == 3 or contador == 6 or contador == 9:
                        print("Acertaste¡,el dinero acumulado es de:" + " " + str(
                            partida.puntaje) + " " + "y estas en seguro")
                    else:
                    ...
                    if contador == 3 or contador == 6 or contador == 9:
                        print("Lo sentimos, no es la respuesta correcta, el dinero obtenido es de:" + " " + str(
                            partida.puntaje))
                        partida.partidaTerminada()
                        historialActual.agregarPartida(partida)
                        saver.save_json_historial(historialActual)
...
def mostrarPartidasGuardadas(registro, cedula):
    ...
    for i in range(0, len(partidas)):
        if partidas[i].jugador.cedula == cedula and partidas[i].estado == True:
            print(str(i + 1) + "." + "partida:" + " " + str(
                partidas[i].codigoPartida) + " " + "puntaje hasta el momento:" + " " + str(partidas[i].puntaje))
            ...
            if contador < 10:
                for partida in partidasJugadas:
                    if len(partida.preguntasAcertadas) == 10:
                        ...
                        resultado = "perdida"
                    print(str(contador + 1) + " . " + "Partida " + resultado + " " + "con una ganancia de: " + str(
                        partida.puntaje))
                    contador = contador + 1
...
def registrarAmigo(registro, jugador):
    ...
    for i in range(0, cantidadAmigos):
        ...
        for i in range(0, 5):
            ...
            conocimiento = input(
                "el nivel de conocimiento de su amigo en la categoria " + categorias[i] + " " + "es de:")
            if conocimiento == "1":
        ...
        if amigo.jugador.cedula == cedula:
            ...
            for i in range(0, len(categorias)):
                if categorias[i] == pregunta.categoria.nombreCategoria:
                    print("Categoria: " + categorias[i] + " " + "conocimiento: " + amigo.nivelConocimientoCategorias[
                        i] + "\n")
